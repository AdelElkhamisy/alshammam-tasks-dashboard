      ...
      <Row gutter={20}>
        ...
        <Col xs={24} md={12}>
          <Card
            ...
            empty={!invitations.data.length}
          >
            {invitations.data.map((item) => (
              <div key={item._id} className="listItem invitation">
                <Row gutter={10} justify="middle">
                  <Col>
                    <Avatar src={getProjectLogo(item.project?.image)} />
                  </Col>
                  <Col>{item.project?.name}</Col>
                </Row>
                <Row>
                  <div onClick={() => dispatch(acceptInvitation(item._id))}>
                    <Icon name="chevron-up success" />
                  </div>
                  <div onClick={() => dispatch(declineInvitation(item._id))}>
                    <Icon name="close danger" />
                  </div>
                </Row>
              </div>
            ))}
