...
const Projects = () => {
  ...
  const createProjectFunc = React.useCallback(() => {
    if (!projects.creating && name)
      dispatch(createProject({ logo, name, members, navigate }));
  }, [logo, name, members, projects]);

  const optsList = React.useMemo(
    () => [
      { value: 1, label: "Active Projects" },
      { value: 2, label: "Archived Projects" },
    ],
    []
  );
    ...
    <div className="projects">
      ...
      <Row gutter={20}>
        ...
        {projects.loading
          ...
          : projects.data
              .filter((p) => (filterType === 2 ? p.archived : !p.archived))
              .map((item) => (
